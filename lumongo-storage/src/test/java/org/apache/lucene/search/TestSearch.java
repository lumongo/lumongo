...
public class TestSearch extends LumongoTestCase {
	...
	public void testNegativeQueryBoost() throws Exception {
		...
		try {
			...
			try {
				...
				assertTrue("score is not negative: " + hits[0].score, hits[0].score < 0);
	...
	private void doTestSearch(Random random, PrintWriter out, boolean useCompoundFile) throws Exception {
		Directory directory = newDirectory();
		Analyzer analyzer = new MockAnalyzer(random);
		IndexWriterConfig conf = newIndexWriterConfig(TEST_VERSION_CURRENT, analyzer);
