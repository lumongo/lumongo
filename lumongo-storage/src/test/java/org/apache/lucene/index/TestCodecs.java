...
public class TestCodecs extends LumongoTestCase {
	...
	public void testFixedPostings() throws Throwable {
		...
		final FieldsProducer reader = codec.postingsFormat().fieldsProducer(
						new SegmentReadState(dir, si, fieldInfos, newIOContext(random()), DirectoryReader.DEFAULT_TERMS_INDEX_DIVISOR));
	...
	public void testRandomPostings() throws Throwable {
		...
		final FieldsProducer terms = codec.postingsFormat().fieldsProducer(
						new SegmentReadState(dir, si, fieldInfos, newIOContext(random()), DirectoryReader.DEFAULT_TERMS_INDEX_DIVISOR));
		...
		public void _run() throws Throwable {
			...
			for (int iter = 0; iter < NUM_TEST_ITER; iter++) {
				...
				for (int i = field.terms.length - 1; i >= 0; i--) {
					assertEquals(Thread.currentThread().getName() + ": field=" + field.fieldInfo.name + " term=" + field.terms[i].text2,
									TermsEnum.SeekStatus.FOUND, termsEnum.seekCeil(new BytesRef(field.terms[i].text2)));
