...
public class MongoDocumentStorage implements DocumentStorage {
	...
	private static final String COMPRESSED_DOC = "_cdoc_";
	...
	public void storeSourceDocument(ResultDocument doc) throws Exception {
		...
		if (!doc.getCompressed()) {
			...
			document.put(COMPRESSED_DOC, bytes);
		}
		...
		if (doc.getMetadataCount() > 0) {
			...
			document.put(ASSOCIATED_METADATA, metadata);
		}
		
		document.put(TIMESTAMP, doc.getTimestamp());
		document.put(COMPRESSED_FLAG, doc.getCompressed());
	...
	public ResultDocument getSourceDocument(String uniqueId, FetchType fetchType) throws Exception {
		if (!FetchType.NONE.equals(fetchType)) {
			...
			if (null != result) {
				boolean compressed = (boolean) result.removeField(COMPRESSED_FLAG);
				...
				if (result.containsField(ASSOCIATED_METADATA)) {
					DBObject metadata = (DBObject) result.removeField(ASSOCIATED_METADATA);
				...
				if (FetchType.FULL.equals(fetchType)) {
					...
					if (!compressed) {
						...
						byte[] bytes = (byte[]) result.get(COMPRESSED_DOC);
